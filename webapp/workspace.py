from __future__ import annotations

import json
import re
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy import func, select
from sqlalchemy.orm import Session, selectinload

from webapp.auth import current_user_required
from webapp.db import get_db
from webapp.models import Scenario, Suite, SuiteEnvValue, User

ROOT = Path(__file__).resolve().parents[1]
EXAMPLES_DIR = ROOT / "examples"
SUITE_FILENAME = "suite.json"
WORKSPACE_PREFIX = "workspace/"
LIBRARY_READONLY = "The public library is read-only. Copy the suite to your workspace to edit."

router = APIRouter(prefix="/api/workspace", tags=["workspace"])


def utcnow() -> datetime:
    return datetime.now(timezone.utc)


def next_owned_suite_position(db: Session, user: User, folder: str = "") -> int:
    current = db.scalar(
        select(func.max(Suite.position)).where(Suite.owner_id == user.id, Suite.folder == folder)
    )
    return int(current or 0) + 1


def is_workspace_path(path: str) -> bool:
    return str(path or "").replace("\\", "/").lstrip("./").startswith(WORKSPACE_PREFIX)


def workspace_suite_path(suite_id: str) -> str:
    return f"workspace/{suite_id}/{SUITE_FILENAME}"


def workspace_scenario_path(suite_id: str, name: str) -> str:
    return f"workspace/{suite_id}/{name}"


def parse_workspace_path(path: str) -> tuple[str, str]:
    rel = str(path or "").strip().replace("\\", "/").lstrip("./")
    if not rel.startswith(WORKSPACE_PREFIX):
        raise HTTPException(status_code=400, detail="Not a workspace path")
    parts = [part for part in rel.split("/") if part]
    if len(parts) != 3 or parts[0] != "workspace":
        raise HTTPException(status_code=400, detail="Workspace path must be workspace/<suite-id>/<file>")
    suite_id, filename = parts[1], parts[2]
    if filename in {".", ".."} or filename.startswith("."):
        raise HTTPException(status_code=400, detail="Invalid workspace filename")
    return suite_id, filename


def safe_filename(value: str) -> str:
    raw = Path(str(value or "").strip().replace("\\", "/")).name
    cleaned = re.sub(r"[^a-zA-Z0-9_.-]+", "_", raw).strip("._")
    if not cleaned:
        cleaned = "scenario"
    if not cleaned.lower().endswith(".json"):
        cleaned += ".json"
    return cleaned


def is_suite_document(payload: dict[str, Any] | None) -> bool:
    if not isinstance(payload, dict):
        return False
    members = payload.get("scenarios")
    steps = payload.get("steps") or []
    return isinstance(members, list) and len(members) > 0 and (not isinstance(steps, list) or len(steps) == 0)


def empty_suite_document(name: str, description: str = "") -> dict[str, Any]:
    return {
        "name": name,
        "description": description,
        "scenarios": [],
        "steps": [],
        "selected_environment": "",
        "environments": {},
        "random_generators": {},
    }


def suite_document_for_client(suite: Suite) -> dict[str, Any]:
    document = dict(suite.document or {})
    document["name"] = suite.name
    document["description"] = suite.description or ""
    document["selected_environment"] = suite.selected_environment or document.get("selected_environment") or ""
    document["scenarios"] = [item.name for item in suite.scenarios]
    document["steps"] = []
    return document


def owned_suite(db: Session, user: User, suite_id: str) -> Suite:
    suite = db.scalar(
        select(Suite)
        .options(selectinload(Suite.scenarios), selectinload(Suite.env_values))
        .where(Suite.id == suite_id, Suite.owner_id == user.id)
    )
    if suite is None:
        raise HTTPException(status_code=404, detail="Suite not found")
    return suite


def apply_suite_fields(suite: Suite, payload: dict[str, Any]) -> None:
    document = dict(payload) if isinstance(payload, dict) else {}
    name = str(document.get("name") or suite.name or "Untitled").strip() or "Untitled"
    suite.name = name
    suite.description = str(document.get("description") or "")
    suite.selected_environment = str(document.get("selected_environment") or "")
    members = document.get("scenarios")
    if not isinstance(members, list):
        members = [item.name for item in suite.scenarios]
    document["name"] = suite.name
    document["description"] = suite.description
    document["selected_environment"] = suite.selected_environment
    document["scenarios"] = [str(item).strip() for item in members if isinstance(item, str) and item.strip()]
    document["steps"] = []
    suite.document = document
    suite.updated_at = utcnow()


def unique_scenario_name(suite: Suite, desired: str) -> str:
    candidate = safe_filename(desired)
    existing = {item.name.lower() for item in suite.scenarios}
    if candidate.lower() not in existing:
        return candidate
    stem = Path(candidate).stem
    for idx in range(2, 1000):
        next_name = f"{stem}_{idx}.json"
        if next_name.lower() not in existing:
            return next_name
    raise HTTPException(status_code=500, detail="Failed to allocate a unique scenario name")


def attach_scenario(db: Session, user: User, suite: Suite, name: str, document: dict[str, Any]) -> Scenario:
    filename = unique_scenario_name(suite, name)
    scenario = Scenario(owner_id=user.id, suite_id=suite.id, name=filename, document=document)
    db.add(scenario)
    members = [item.name for item in suite.scenarios]
    if filename not in members:
        members.append(filename)
    payload = dict(suite.document or {})
    payload["scenarios"] = members
    suite.document = payload
    suite.updated_at = utcnow()
    db.flush()
    return scenario


def workspace_tree(user: User, db: Session) -> dict[str, Any]:
    suites = db.scalars(
        select(Suite)
        .options(selectinload(Suite.scenarios))
        .where(Suite.owner_id == user.id)
        .order_by(Suite.updated_at.desc())
    ).all()
    children: list[dict[str, Any]] = []
    for suite in suites:
        members = [item.name for item in suite.scenarios]
        children.append({
            "type": "file",
            "kind": "suite",
            "name": suite.name or SUITE_FILENAME,
            "path": workspace_suite_path(suite.id),
            "source": "workspace",
            "base_url": (suite.document or {}).get("base_url"),
            "step_count": 0,
            "member_count": len(members),
            "members": members,
        })
    return {
        "type": "dir",
        "name": "My workspace",
        "path": "workspace",
        "source": "workspace",
        "children": children,
    }


def _read_examples_json(path: Path) -> dict[str, Any]:
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Invalid scenario JSON: {exc}") from exc
    if not isinstance(payload, dict):
        raise HTTPException(status_code=400, detail="Scenario JSON must be an object")
    return payload


def _normalize_examples_rel(relative: str) -> str:
    raw = (relative or "").strip().replace("\\", "/")
    parts: list[str] = []
    for part in raw.split("/"):
        if part in ("", "."):
            continue
        if part == ".." or part.startswith("."):
            raise HTTPException(status_code=400, detail="Invalid path")
        parts.append(part)
    if not parts:
        raise HTTPException(status_code=400, detail="Invalid path")
    return "/".join(parts)


def resolve_examples_path(relative: str) -> Path:
    rel = _normalize_examples_rel(relative)
    base = EXAMPLES_DIR.resolve()
    path = (base / rel).resolve()
    try:
        path.relative_to(base)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Invalid path") from exc
    return path


def clone_library_path(db: Session, user: User, library_path: str) -> Suite:
    file_path = resolve_examples_path(library_path)
    if not file_path.is_file() or file_path.suffix.lower() != ".json":
        raise HTTPException(status_code=404, detail="Library file not found")
    payload = _read_examples_json(file_path)
    source_dir = file_path.parent
    if not is_suite_document(payload):
        from tools.scenario_runner import discover_parent_suite

        parent = discover_parent_suite(file_path, payload)
        if isinstance(parent, dict) and parent.get("_file"):
            parent_path = source_dir / str(parent["_file"])
            if parent_path.is_file():
                payload = _read_examples_json(parent_path)
                source_dir = parent_path.parent
            else:
                payload = {
                    **empty_suite_document(str(payload.get("name") or file_path.stem)),
                    "environments": payload.get("environments") or {},
                    "random_generators": payload.get("random_generators") or {},
                    "selected_environment": payload.get("selected_environment") or "",
                    "scenarios": [file_path.name],
                }
        else:
            payload = {
                **empty_suite_document(str(payload.get("name") or file_path.stem)),
                "environments": payload.get("environments") or {},
                "random_generators": payload.get("random_generators") or {},
                "selected_environment": payload.get("selected_environment") or "",
                "scenarios": [file_path.name],
            }
            source_dir = file_path.parent

    suite = Suite(
        owner_id=user.id,
        name=str(payload.get("name") or source_dir.name or "Untitled"),
        description=str(payload.get("description") or ""),
        selected_environment=str(payload.get("selected_environment") or ""),
        folder="",
        position=next_owned_suite_position(db, user, ""),
        document=payload,
    )
    db.add(suite)
    db.flush()
    members = payload.get("scenarios") if isinstance(payload.get("scenarios"), list) else []
    cloned_names: list[str] = []
    for member in members:
        if not isinstance(member, str) or not member.strip():
            continue
        member_name = safe_filename(Path(member).name)
        member_path = source_dir / Path(member).name
        document = _read_examples_json(member_path) if member_path.is_file() else {"steps": []}
        db.add(Scenario(owner_id=user.id, suite_id=suite.id, name=member_name, document=document))
        cloned_names.append(member_name)
    payload = dict(payload)
    payload["scenarios"] = cloned_names
    apply_suite_fields(suite, payload)
    db.flush()
    db.refresh(suite)
    return suite


def string_env_map(values: Any) -> dict[str, str]:
    if not isinstance(values, dict):
        return {}
    cleaned: dict[str, str] = {}
    for key, value in values.items():
        name = str(key or "").strip()
        if not name:
            continue
        text = "" if value is None else str(value).strip()
        if text:
            cleaned[name] = text
    return cleaned


def load_suite_env_values(db: Session, user: User, suite_id: str, environment: str) -> dict[str, str]:
    row = db.scalar(
        select(SuiteEnvValue).where(
            SuiteEnvValue.suite_id == suite_id,
            SuiteEnvValue.owner_id == user.id,
            SuiteEnvValue.environment_name == environment,
        )
    )
    return string_env_map(row.values if row else {})


def upsert_suite_env_values(db: Session, user: User, suite: Suite, environment: str, values: Any) -> dict[str, str]:
    cleaned = string_env_map(values)
    row = db.scalar(
        select(SuiteEnvValue).where(
            SuiteEnvValue.suite_id == suite.id,
            SuiteEnvValue.owner_id == user.id,
            SuiteEnvValue.environment_name == environment,
        )
    )
    if row is None:
        row = SuiteEnvValue(
            owner_id=user.id,
            suite_id=suite.id,
            environment_name=environment,
            values=cleaned,
        )
        db.add(row)
    else:
        row.values = cleaned
        row.updated_at = utcnow()
    return cleaned


def materialize_workspace_run(db: Session, user: User, path: str, dest_dir: Path) -> Path:
    suite_id, filename = parse_workspace_path(path)
    suite = owned_suite(db, user, suite_id)
    if filename == SUITE_FILENAME or filename == safe_filename(suite.name):
        suite_path = dest_dir / SUITE_FILENAME
        suite_path.write_text(json.dumps(suite_document_for_client(suite), indent=2), encoding="utf-8")
        for scenario in suite.scenarios:
            (dest_dir / scenario.name).write_text(
                json.dumps(scenario.document or {}, indent=2),
                encoding="utf-8",
            )
        return suite_path
    scenario = next((item for item in suite.scenarios if item.name == filename), None)
    if scenario is None:
        raise HTTPException(status_code=404, detail="Scenario not found")
    scenario_path = dest_dir / scenario.name
    scenario_path.write_text(json.dumps(scenario.document or {}, indent=2), encoding="utf-8")
    return scenario_path


@router.get("/file")
def get_workspace_file(
    path: str = Query(..., min_length=1),
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    suite_id, filename = parse_workspace_path(path)
    suite = owned_suite(db, user, suite_id)
    if filename == SUITE_FILENAME:
        return suite_document_for_client(suite)
    scenario = next((item for item in suite.scenarios if item.name == filename), None)
    if scenario is None:
        raise HTTPException(status_code=404, detail="Scenario not found")
    return dict(scenario.document or {})


@router.post("/file")
def save_workspace_file(
    payload: dict[str, Any],
    path: str = Query(..., min_length=1),
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, str]:
    if not isinstance(payload, dict):
        raise HTTPException(status_code=400, detail="Document must be an object")
    suite_id, filename = parse_workspace_path(path)
    suite = owned_suite(db, user, suite_id)
    if filename == SUITE_FILENAME or is_suite_document(payload):
        apply_suite_fields(suite, payload)
        return {"status": "saved", "path": workspace_suite_path(suite.id)}
    filename = safe_filename(filename)
    scenario = next((item for item in suite.scenarios if item.name == filename), None)
    if scenario is None:
        scenario = attach_scenario(db, user, suite, filename, payload)
    else:
        scenario.document = payload
        scenario.updated_at = utcnow()
        suite.updated_at = utcnow()
    return {"status": "saved", "path": workspace_scenario_path(suite.id, scenario.name)}


@router.get("/parent-suite")
def workspace_parent_suite(
    path: str = Query(..., min_length=1),
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    suite_id, filename = parse_workspace_path(path)
    suite = owned_suite(db, user, suite_id)
    if filename == SUITE_FILENAME:
        return {"status": "none"}
    document = suite_document_for_client(suite)
    return {
        "status": "ok",
        "path": workspace_suite_path(suite.id),
        "name": suite.name,
        "description": suite.description or "",
        "selected_environment": suite.selected_environment or "",
        "environments": document.get("environments") if isinstance(document.get("environments"), dict) else {},
        "random_generators": document.get("random_generators") if isinstance(document.get("random_generators"), dict) else {},
        "scenarios": document.get("scenarios") if isinstance(document.get("scenarios"), list) else [],
    }


@router.post("/suites")
def create_suite(
    payload: dict[str, Any] | None = None,
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    body = payload if isinstance(payload, dict) else {}
    name = str(body.get("name") or "Untitled").strip() or "Untitled"
    folder = str(body.get("folder") or "").strip()
    document = body.get("document") if isinstance(body.get("document"), dict) else empty_suite_document(name, str(body.get("description") or ""))
    suite = Suite(
        owner_id=user.id,
        name=name,
        description=str(document.get("description") or body.get("description") or ""),
        selected_environment=str(document.get("selected_environment") or ""),
        folder=folder,
        position=next_owned_suite_position(db, user, folder),
        document=document,
    )
    db.add(suite)
    db.flush()
    apply_suite_fields(suite, {**document, "name": name})
    return {"status": "created", "id": suite.id, "path": workspace_suite_path(suite.id), "name": suite.name}


@router.post("/clone")
def clone_from_library(
    payload: dict[str, Any],
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    source = str(payload.get("path") or "").strip()
    if not source:
        raise HTTPException(status_code=400, detail="path is required")
    if is_workspace_path(source):
        raise HTTPException(status_code=400, detail="Path is already in your workspace")
    suite = clone_library_path(db, user, source)
    return {
        "status": "cloned",
        "id": suite.id,
        "path": workspace_suite_path(suite.id),
        "name": suite.name,
        "scenarios": [item.name for item in suite.scenarios],
    }


@router.delete("/item")
def delete_item(
    path: str = Query(..., min_length=1),
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    if is_workspace_path(path):
        return delete_workspace_item(db, user, path)
    return delete_library_item(path)


def delete_workspace_item(db: Session, user: User, path: str) -> dict[str, Any]:
    suite_id, filename = parse_workspace_path(path)
    suite = owned_suite(db, user, suite_id)
    if filename == SUITE_FILENAME:
        db.delete(suite)
        return {"status": "deleted", "kind": "suite", "path": path}
    scenario = next((item for item in suite.scenarios if item.name == filename), None)
    if scenario is None:
        raise HTTPException(status_code=404, detail="Scenario not found")
    db.delete(scenario)
    remaining = [item.name for item in suite.scenarios if item.name != filename]
    payload = dict(suite.document or {})
    payload["scenarios"] = remaining
    apply_suite_fields(suite, payload)
    return {"status": "deleted", "kind": "scenario", "path": path, "suite_path": workspace_suite_path(suite.id)}


def delete_library_item(path: str) -> dict[str, Any]:
    file_path = resolve_examples_path(path)
    if not file_path.is_file() or file_path.suffix.lower() != ".json":
        raise HTTPException(status_code=404, detail="Library file not found")
    folder = file_path.parent
    if folder.resolve() == EXAMPLES_DIR.resolve():
        file_path.unlink()
        return {"status": "deleted", "kind": "file", "path": path}

    payload = _read_examples_json(file_path)
    if file_path.name == SUITE_FILENAME or is_suite_document(payload):
        members = payload.get("scenarios") if isinstance(payload.get("scenarios"), list) else []
        for member in members:
            if not isinstance(member, str) or not member.strip():
                continue
            child = folder / Path(member).name
            if child.is_file() and child != file_path:
                child.unlink()
        file_path.unlink()
        try:
            next(folder.iterdir())
        except StopIteration:
            folder.rmdir()
        return {"status": "deleted", "kind": "suite", "path": path}

    file_path.unlink()
    suite_path = folder / SUITE_FILENAME
    if suite_path.is_file():
        suite = _read_examples_json(suite_path)
        members = suite.get("scenarios") if isinstance(suite.get("scenarios"), list) else []
        suite["scenarios"] = [
            name for name in members if isinstance(name, str) and Path(name).name != file_path.name
        ]
        suite_path.write_text(json.dumps(suite, indent=2) + "\n", encoding="utf-8")
    return {"status": "deleted", "kind": "scenario", "path": path, "suite_path": str(suite_path.relative_to(EXAMPLES_DIR)).replace("\\", "/")}


@router.get("/suites/{suite_id}/env")
def get_suite_env(
    suite_id: str,
    environment: str = Query("", alias="environment"),
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    suite = owned_suite(db, user, suite_id)
    env_name = environment or suite.selected_environment or ""
    return {"environment": env_name, "values": load_suite_env_values(db, user, suite.id, env_name)}


@router.put("/suites/{suite_id}/env")
def put_suite_env(
    suite_id: str,
    payload: dict[str, Any],
    user: User = Depends(current_user_required),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    suite = owned_suite(db, user, suite_id)
    env_name = str(payload.get("environment") or suite.selected_environment or "").strip()
    if not env_name:
        raise HTTPException(status_code=400, detail="environment is required")
    values = upsert_suite_env_values(db, user, suite, env_name, payload.get("values"))
    return {"status": "saved", "environment": env_name, "values": values}
